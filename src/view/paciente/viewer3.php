    ...
    <section class="content">
        <div class="container-fluid">
            <div class="row">
                ...
                <!-- Panel derecho -->
                <div class="col-sm-6" id="jsonDisplay">
                    <h1></h1>
                </div>
...
<script>
    document.addEventListener("DOMContentLoaded", () => {
        ...
        if (bundleId) {
            ...
            fetch(url)
                .then(response => {
                    if (!response.ok) throw new Error("Error al obtener el Bundle");
                    return response.json();
                })
                .then(data => {
                    // ✅ Mostrar JSON crudo en el editor CodeMirror
                    editor.setValue(JSON.stringify(data, null, 2));

                    // ✅ Guardar el JSON en el textarea oculto (para processJSON)
                    document.getElementById("jsonInput").value = JSON.stringify(data);

                    // ✅ Llamar al visor para renderizar tarjetas
                    processJSON();
                })
                .catch(error => {
                    console.error("Error:", error);
                    document.getElementById("jsonDisplay").innerHTML =
                        `<p style="color:red;">No se pudo cargar el Bundle: ${error.message}</p>`;
                });
        }
    });
   ...
   function displayPatientInfo(patient, organization, composition, container) {
    ...
    const birthDate = patient?.birthDate ? new Date(patient.birthDate + 'T00:00:00Z') : null;
    const formattedDate = birthDate
        ? `${birthDate.getUTCDate().toString().padStart(2, '0')}/${(birthDate.getUTCMonth() + 1).toString().padStart(2, '0')}/${birthDate.getUTCFullYear()}`
        : 'No disponible';
    const age = birthDate ? new Date().getFullYear() - birthDate.getUTCFullYear() : 'No disponible';

    const organizationName = organization?.name || 'No disponible';
    const organizationCountry = organization?.address?.[0]?.country || 'No disponible';
